...
def check_unanswered_messages(browser, user):
    ...
    try:
        ...
        if profile_is_delete.text == 'Анкета пользователя удалена.':
            ...
            print(
                ...
                'вызывая функцию update_user_data из блока except '
                'функции sesd_messages'
            )
    ...
    try:
        ...
        if 'incoming' in message.get_dom_attribute('class'):
            print(
                f'В чате с {name} по ссылке {browser.current_url} '
                'есть неотвеченное сообщение'
            )
        ...
        print(
            f'Ошибка при проверке статуса сообщения: {e} '
            f'в чате {browser.current_url}')
